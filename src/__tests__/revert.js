...
it('accepts only boolean values in camelCase option', () => {
  expect(() => revert('', { camelCase: true })).not.toThrow()
  expect(() => revert('', { camelCase: false })).not.toThrow()
  expect(() => revert('', { camelCase: null })).toThrow(
    'camelCase must be a boolean'
  )
})

it('accepts an empty string as separator', () => {
  expect(() => revert('', { separator: '' })).not.toThrow()
})

it('accepts null as separator', () => {
  expect(() => revert('', { separator: null })).not.toThrow()
})

it('allows only accepted separator characters', () => {
  expect(() => revert('', { separator: "-._~!$&'()*+,;=" })).not.toThrow()
  expect(() => revert('', { separator: 'x' })).toThrow(
    'separator has an invalid character'
  )
})

it('does not accept a separator that is not a string', () => {
  expect(() => revert('', { separator: 123 })).toThrow(
    'separator must be a string or null'
  )
})

it('accepts false as a transformer', () => {
  expect(() => revert('', { transformer: false })).not.toThrow()
})

it('accepts a function as a transformer', () => {
  expect(() => revert('', { transformer: () => {} })).not.toThrow()
})

it('accepts only false or a function as a transformer', () => {
  expect(() => revert('', { transformer: true })).toThrow(
    'transformer must be false or a function'
  )
  expect(() => revert('', { transformer: 'string' })).toThrow(
    'transformer must be false or a function'
  )
  expect(() => revert('', { transformer: {} })).toThrow(
    'transformer must be false or a function'
  )
})
